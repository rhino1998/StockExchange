...
public class StockExchangeEndpoint {
    ...
    public Response getQuoteDescription(@PathParam("exchange") String exchange,
        ...
        GenericEntity<String> entity = new GenericEntity<String>(
                stock.getDescription(), String.class);
        return Response.ok().entity(entity).build();
    }
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public Response getQuoteHistory(@PathParam("exchange") String exchange,
            @PathParam("symbol") String symbol, @PathParam("offset") long offset) {

        List<StockDataPoint> history = StockMarket.getStockExchange(exchange)
                .getStock(symbol).getHistory().getAll();
        int i = 0;
